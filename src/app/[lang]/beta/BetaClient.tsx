...
export default function BetaClient({ dict, dictNav, lang }: { dict: any; dictNav: any; lang: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3457/v1";
            const response = await fetch(`${apiUrl}/beta/apply`, {
                method: "POST",
                headers: {
            ...
            alert(dict.error || "An error occurred. Please try again.");
        } finally {
            setLoading(false);
        }
    };

    const isValid = Boolean(form.email.trim() && form.platform);
                ...
                <div className="container">
                    <div className="apply-content">
                        ...
                        <div className="apply-header">
                            ...
                            <p className="apply-subtitle">{dict.subtitle}</p>
                        </div>
                        ...
                        {submitted ? (
                            ...
                            <form className="apply-form" onSubmit={handleSubmit}>
                                ...
                                <div className="form-submit-area" style={{ marginTop: "2rem" }}>
                                    <p className="form-note">{dict.note}</p>
                                    <button
                                        id="beta-submit-btn"
                                        type="submit"
                                    ...
                                    <p className="form-privacy">{dict.privacy}</p>
                                </div>
                            </form>
                        )}
                    </div>
